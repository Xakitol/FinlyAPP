...
export function AddTransactionModal({ open, onClose, darkMode = false, onSave }: AddTransactionModalProps) {
  ...
  // Persisted custom categories + usage counts
  ...
  // Build sorted category lists (high usage first)
  ...
  // Auto-detect type from chosen category
  ...
  function handleSave() {
    if (!numericAmount || !description.trim() || !category) return;
    const mem = loadDescMemory();
    const updated = recordTransaction(mem, description.trim(), type, category);
  ...
  const galaxyBtnPressed: CSSProperties = {
    boxShadow: '0 1px 0 rgba(124,58,237,0.5), inset 0 1px 0 rgba(255,255,255,0.2)',
  };
    ...
    return (
      <div className="flex items-center justify-between px-5 pt-5 pb-3 shrink-0">
        ...
        <div className="text-center">
          <StepDots />
          <h2 className={`text-[16px] font-bold ${text}`}>{title}</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
  ...
  function Step1() {
    ...
    return (
      <div className="flex flex-col flex-1 px-5 pb-6 gap-3" style={contentStyle}>
        ...
        >
          <p className={`text-[11px] font-semibold tracking-wide mb-1 ${darkMode ? 'text-white/40' : 'text-gray-400'}`}>
            כמה?
          </p>
        ...
        <div className="grid grid-cols-3 gap-2">
          {NUMPAD_KEYS.map((key) => (
            <button
              ...
              className={`h-14 rounded-2xl text-[22px] font-semibold flex items-center justify-center ${text}`}
              style={{
        ...
        <button
          ...
          onPointerDown={canNext ? tactilePress : undefined}
          onPointerUp={canNext ? tactileRelease : undefined}
          onPointerLeave={canNext ? tactileRelease : undefined}
        >
          הבא ←
        </button>
      </div>
  ...
  function Step2() {
    const canNext = description.trim().length > 0 && category !== '';
    ...
    function CatCard({ cat, active }: { cat: CategoryDef; active: boolean }) {
      ...
      return (
        <button
          ...
          style={{
            ...
            border: active
              ? (isExpense ? '1.5px solid rgba(168,85,247,0.5)' : '1.5px solid rgba(14,165,233,0.5)')
              : (darkMode ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(200,190,255,0.35)'),
            boxShadow: active
              ? (isExpense ? '0 4px 0 rgba(168,85,247,0.25)' : '0 4px 0 rgba(14,165,233,0.2)')
        ...
        >
          <cat.Icon
            className="h-5 w-5"
            style={{ color: active ? (isExpense ? '#a855f7' : '#0ea5e9') : (darkMode ? 'rgba(255,255,255,0.6)' : '#6b7280') }}
          />
          <p className={`text-[11px] font-semibold leading-tight ${active ? (isExpense ? 'text-purple-500' : 'text-sky-500') : (darkMode ? 'text-white/80' : 'text-gray-700')}`}>
            {cat.label}
          </p>
    ...
    return (
      <div className="flex flex-col flex-1 overflow-hidden" style={contentStyle}>
        ...
        <div className="px-5 pb-3 relative">
          <p className={`text-[10px] font-semibold mb-1.5 ${muted}`}>שם התנועה</p>
          <input
            ref={descInputRef}
            type="text"
            value={description}
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {sortedCats(EXPENSE_CATS).map((cat) => (
              <CatCard key={cat.id} cat={cat} active={category === cat.id} />
            ))}
  ...
  function Step3() {
    function SelectCard({
      ...
      return (
        <button
          ...
          style={{
            background: active ? gradient : (darkMode ? 'rgba(255,255,255,0.07)' : 'rgba(255,255,255,0.8)'),
            border: active ? 'none' : (darkMode ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(200,190,255,0.4)'),
            boxShadow: active ? shadow : (darkMode ? '0 5px 0 rgba(0,0,0,0.3)' : '0 5px 0 rgba(180,170,220,0.25)'),
            transition: 'all 0.15s ease',
          }}
          onPointerDown={tactilePress}
          onPointerUp={tactileRelease}
    ...
    const summaryAmount = `₪${numericAmount.toLocaleString('he-IL', { maximumFractionDigits: 2 })}`;
    ...
    return (
      <div className="flex flex-col flex-1 px-5 pb-6 gap-4" style={contentStyle}>
        ...
        <div
          ...
          style={{ background: darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(99,102,241,0.06)', border: darkMode ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(99,102,241,0.1)' }}
        >
          <span className={`text-[13px] font-bold ${darkMode ? 'text-white/80' : 'text-gray-700'}`}>{summaryCategory}</span>
          <span className={`text-[17px] font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{summaryAmount}</span>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <SelectCard
              ...
              shadow="0 5px 0 rgba(168,85,247,0.4)"
            >
            ...
            <SelectCard
              ...
              shadow="0 5px 0 rgba(14,165,233,0.4)"
            >
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <SelectCard
              ...
              shadow="0 5px 0 rgba(2,132,199,0.4)"
            >
            ...
            <SelectCard
              ...
              shadow="0 5px 0 rgba(124,58,237,0.4)"
            >
        ...
        <button
          ...
          onPointerDown={(e) => {
            ...
            e.currentTarget.style.boxShadow = '0 1px 0 rgba(99,102,241,0.55)';
          }}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ ...modalBg, maxHeight: '92vh', minHeight: '70vh' }}
      >
        <Header title={stepTitles[step]} />
        {step === 1 && <Step1 />}
        {step === 2 && <Step2 />}
        {step === 3 && <Step3 />}
      </div>
