...
import { X, Upload, CheckCircle, AlertCircle, HelpCircle, FileText } from 'lucide-react';
...
type Stage = 'idle' | 'parsing' | 'preview' | 'done';
...
export function ImportModal({ open, onClose, darkMode = false, onImport }: ImportModalProps) {
  ...
  function updateRowType(id: string, type: 'income' | 'expense') {
    setRows((prev) =>
      prev.map((r) =>
        r.id === id
          ? { ...r, type, confidence: 'suggestion', needsReview: false }
          : r,
      ),
    );
  }
          ...
          <div className="px-5 pb-6">
            {stage === 'parsing' ? (
              ...
              <>
                ...
                {error && (
                  <p className={`mt-3 text-center text-[12px] ${darkMode ? 'text-fuchsia-300' : 'text-fuchsia-600'}`}>
                    {error}
                  </p>
                )}
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p className={`mt-1 text-[12px] ${muted}`}>כל הרשומות נשמרו בחשבון שלכם</p>
          <button
            type="button"
            onClick={handleClose}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-3 pt-5 shrink-0">
          ...
          <div className="text-right">
            ...
            <p className={`text-[11px] ${muted}`}>
              ...
              {ambiguousCount > 0 ? ` · ${ambiguousCount} דורשות בחירת סוג` : ''}
            </p>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <p className={`text-[13px] font-semibold text-right truncate flex-1 ${text}`}>
                      {row.description}
                    </p>
                  </div>

                  {/* Meta row: date + category + confidence */}
                  ...
                  {/* Type toggle for ambiguous rows */}
                  {isAmbiguous && (
                    <div className="flex gap-2 mt-2.5">
                      <button
                        ...
                        className={`flex-1 rounded-xl py-1.5 text-[11px] font-semibold transition-all ${
                          row.type === 'expense' ? 'text-white' : darkMode ? 'text-white/40' : 'text-gray-400'
                        }`}
                        style={{
                          background: row.type === 'expense'
                      ...
                      <button
                        ...
                        className={`flex-1 rounded-xl py-1.5 text-[11px] font-semibold transition-all ${
                          row.type === 'income' ? 'text-white' : darkMode ? 'text-white/40' : 'text-gray-400'
                        }`}
                        style={{
        ...
        <div
          ...
          style={{
            borderTop: darkMode ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(139,92,246,0.1)',
          }}
        >
          <button
            type="button"
